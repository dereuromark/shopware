    ...
    private function createWebhookSection(): ArrayNodeDefinition
    {
        ...
        $rootNode
            ->children()
                ...
                ->end()
                ->arrayNode('health')
                    ->info('@experimental feature:WEBHOOKS_REWORK — Phase 2 endpoint-health (circuit breaker) tuning.')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->arrayNode('cooldown_schedule_seconds')
                            ->info('DEGRADED probe cooldown tiers in seconds (~doubling backoff, 5m→4h cap); length must equal max_degraded_cycles.')
                            ->integerPrototype()->min(1)->end()
                            ->defaultValue([300, 600, 1200, 2400, 3600, 14400])
                        ->end()
                        ->integerNode('max_degraded_cycles')->min(1)->defaultValue(6)->end()
                        ->integerNode('degraded_threshold')->min(1)->defaultValue(5)->end()
                        ->integerNode('max_suspended_days')->min(1)->max(14)->defaultValue(7)->end()
                    ->end()
                    ->validate()
                        ->ifTrue(static fn (array $v): bool => !\is_array($v['cooldown_schedule_seconds']) || \count($v['cooldown_schedule_seconds']) !== $v['max_degraded_cycles'])
                        ->thenInvalid('webhook.health.cooldown_schedule_seconds length must equal max_degraded_cycles')
                    ->end()
                ->end()
            ->end();
